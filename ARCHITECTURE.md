...
- `NodeVmSandbox` today — Node's `vm` module. Each test case is compiled and invoked as its own timed `vm.Script`, so a submission that returns is bounded per-case, not just at function-definition time.
- **Known limitation, not a bug:** `vm`'s timeout stops a busy loop, but Node is single-threaded — while a submission is spinning, it blocks the whole server process for up to `DSA_SANDBOX_TIMEOUT_MS`, so no other request (including your own next click) completes until it times out. Fine for a single local user running their own code; not a boundary for untrusted or concurrent use.
